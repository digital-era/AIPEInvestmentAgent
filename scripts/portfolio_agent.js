...
let agents = {
    agent1: {
        ...
        addToPortfolioInputId: 'agent1AddToPortfolioInput', // For manual input
        addFromPoolSelectId: 'agent1AddFromPoolSelect',    // For dropdown add
...
6.  **量化因子**: 从标的动能(PotScore)、主力资金流入情况两个角度分析量化因素 (请在您的分析中体现对这些因素的考量，例如：近期${stockName}的PotScore为[示例值]，主力资金连续[示例天数]日净流入，这表明...)。
...
注意事项：
回答信息中不要带有任何Warren Buffett字眼，这里全称都体现你自己
...
请你作为${agents.agent1.name} (${agents.agent1.role})，开始你的思考和中文回复:
    ...
    agent2: { 
        ...
        addToPortfolioInputId: 'agent2AddToPortfolioInput', // For manual input
        addFromPoolSelectId: 'agent2AddFromPoolSelect',     // For dropdown add
...
6.  **量化因子**: 从标的动能(PotScore)、主力资金流入情况两个角度分析量化因素 (请在您的分析中体现对这些因素的考量，例如：${stockName}近期的PotScore趋势如何，主力资金是否持续流入支持其成长故事？...)。
...
const modelDisplayStrings = { 
    ...
    "modelGeminiFlash": "Google Gemini 1.5 Flash (gemini-1.5-flash-preview-0514)",
...
function findStockInGlobalData(searchTerm) {
    ...
    if (allStockData[searchTerm]) { 
        foundStock = { code: searchTerm, name: allStockData[searchTerm].name };
    } else if (allStockData[upperSearchTerm]) { 
         foundStock = { code: upperSearchTerm, name: allStockData[upperSearchTerm].name };
    } else {
...
// Stock Analysis For Agent 
function analyzeStockForAgent(agentId) {
    const agent = agents[agentId];
    ...
    let actualStockNameForPrompt = stockToAnalyze.split('(')[0].trim(); 

    if(analysisOutput) {
        analysisOutput.value = `正在生成关于 "${actualStockNameForPrompt}" 的分析报告...\n\n`;
        analysisOutput.value += `--- PROMPT PREVIEW (${agent.name} - ${agent.role}) ---\n`;
        analysisOutput.value += agent.promptTemplate(actualStockNameForPrompt);
        analysisOutput.value += "\n\n--- SIMULATED RESPONSE ---";
        analysisOutput.value += `\n对于 ${actualStockNameForPrompt}，基于我的${agent.role}理念，初步分析和配置思路如下：\n`;
        analysisOutput.value += `[此处为详细分析，包括基本面、行业前景、管理层、护城河/创新点等]\n`;
        analysisOutput.value += `量化因子方面：\n`;
        analysisOutput.value += `  - 标的动能 (PotScore): [模拟PotScore为 0.75, 趋势为 上升]\n`;
        analysisOutput.value += `  - 主力资金流入: [模拟主力资金 连续3日净流入]\n`;
        analysisOutput.value += `这些量化指标[支持]我的定性判断。\n`;
        analysisOutput.value += `配置思路：[例如：可考虑在组合中初步配置5-10%仓位，后续根据市场及公司发展调整...]\n\n`;
        analysisOutput.value += `(注意: 这仅为模拟回复。实际分析需接入AI模型并根据实时数据进行。)`;
    }
...
function clearApiSettings() { // Function to clear API settings
    if (confirm("确定要清除所有已保存的API设置吗？此操作不可撤销。")) {
        localStorage.removeItem('apiSettings');
        apiSettings = { endpoint: '', key: '', model: '' }; // Reset in-memory object
...
function loadApiSettings() {
    ...
    endpointSelect.addEventListener('change', (event) => {
        ...
        populateApiModelDropdown(newSelectedEndpoint); 
    });

    const savedSettings = localStorage.getItem('apiSettings');
    let initialEndpointToLoad = firstEndpointValue; 
